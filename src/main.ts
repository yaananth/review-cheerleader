...
async function run(): Promise<void> {
    ...
    const slackWebHookMap: { [key : string]: string} = JSON.parse((core.getInput('slack-webhook-url-map') || '').replace(/'/g, '"'))
    ...
    let pullsResult: any
    let outputTeamToAuthorText: { [key : string]: { [key : string]: IAuthorStats}} = {}
    ...
    try {
        ...
        if (Object.keys(slackWebHookMap).length > 0 && Object.keys(state.additionalDetails).length > 0) {
            ...
            for (const teamNameIndex in teamNamesArray) {
                ...
                if (state.additionalDetails[teamName]) {
                    ...
                    let message = slackHeadingEmoji + ` *I am your review cheer leader! I have an update for you!*`
                    message += `\n\n`
...
async function getStoredState(octokit: InstanceType<typeof GitHub>): Promise<State> {
    let store = {
        lastPRCursor: ''
    } as State
